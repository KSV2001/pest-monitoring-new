...
from src.data.COCO.utils import (
    cxcy_to_gcxgcy,
    cxcy_to_xy,
...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predicted_locs, predicted_scores, boxes, labels):
        ...
        :param predicted_locs: predicted locations/boxes w.r.t the
            8732 prior boxes, a tensor of dimensions (N, 8732, 4)
        ...
        for i in range(batch_size):
            ...
            # To remedy this -
            # First, find the prior that has the maximum overlap
            #   for each object.
            ...
            # To ensure these priors qualify, artificially give them
            #   an overlap of greater than 0.5. (This fixes 2.)
            ...
            # Encode center-size object coordinates into the form
            #   we regressed predicted boxes to
        ...
        # Note: indexing with a torch.uint8 (byte) tensor flattens
        #   the tensor when indexing is across multiple dimensions (N & 8732)
        # So, if predicted_locs has the shape (N, 8732, 4),
        #   predicted_locs[positive_priors] will have (total positives, 4)

        # CONFIDENCE LOSS

        # Confidence loss is computed over positive priors and the
        #   most difficult (hardest) negative priors in each image
        #   That is, FOR EACH IMAGE, we will take the hardest
        #   (neg_pos_ratio * n_positives) negative priors, i.e where
        #   there is maximum loss
        # This is called Hard Negative Mining - it concentrates on
        ...
        # To do this, sort ONLY negative priors in each image in
        #   order of decreasing loss and take top n_hard_negatives
        ...
        hardness_ranks = (
            torch.LongTensor(range(n_priors)).unsqueeze(0).expand_as(conf_loss_neg)
        )  # (N, 8732)
        hard_negatives = hardness_ranks < n_hard_negatives.unsqueeze(1)  # (N, 8732)
        conf_loss_hard_neg = conf_loss_neg[hard_negatives]  # (sum(n_hard_negatives))

        # As in the paper, averaged over positive priors only, although
        # computed over both positive and hard-negative priors
        conf_loss = (
